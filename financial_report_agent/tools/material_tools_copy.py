...
def _save_df_to_material(
    ...
    if short_term is not None:
        ...
        short_term.save_material(ref_id=ref_id, content=csv_text, ext="csv")
    return len(df)
...
def _read_df_from_material(
    ...
    csv_text = short_term.load_material(ref_id=ref_id, ext="csv")
    if not csv_text:
        return None
...
async def fetch_realtime_price_material(
    ...
    _save_df_to_material(df, ref_id, short_term)
...
async def fetch_top10_float_shareholders_material(
    ...
    Args:
        symbol (str):
            带市场标识的股票代码，例如 "sh688686"。
        date (str):
            财报发布季度最后一日，格式为 "YYYYMMDD"，例如 "20240930"。
        ref_id (Optional[str]):
...
async def fetch_top10_shareholders_material(
    ...
    Args:
        symbol (str):
            带市场标识的股票代码，例如 "sh688686"。
        date (str):
            财报发布季度最后一日，格式为 "YYYYMMDD"，例如 "20210630"。
        ref_id (Optional[str]):
...
def read_table_material(
    ...
    """
    df = short_term.load_material(ref_id=ref_id, ext="csv")
