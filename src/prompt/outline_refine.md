...
## evidence 字段
每条 evidence 是一个对象，至少包含 `text` 或 `description`，推荐包含以下结构化字段：

- `text`：
  这条论据要搜索什么。应尽量具体、清楚、可检索。可以与 `description` 相同。
- `description`：
  结构化论据描述，应能独立作为检索需求。
- `entity`：
  论据主体，例如公司、行业、业务板块。
- `aspect`：
  指标、事件或分析维度，例如营业收入同比增速、毛利率变化、增长原因。
- `period`：
  时间范围，例如上一季度、近三年、2025FY。
- `scope`：
  统计口径或业务范围，例如公司整体、海外业务。
- `required`：
  是否为当前 segment 必需证据，布尔值。
