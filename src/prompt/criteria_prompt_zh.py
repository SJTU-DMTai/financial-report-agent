...
<system_role>
你是一名经验丰富的调研文章评估专家。你擅长评估文章中的论据是否足够支持论点。
</system_role>

<user_prompt>
**背景**：我们正在评估一篇针对以下调研任务撰写的深度调研文章。
<task>
"{task_prompt}"
</task>

<instruction>
**你的目标**：为这篇调研文章的 **论据充分性 (Sufficiency)** 维度，制定一套详细、具体、且高度针对上述 `<task>` 的评判标准。你需要：
1.  **分析任务**: 深入分析 `<task>`，识别出要支持文章中的论点，需要哪些方面的论据，以及论据的数量和质量要求。
2.  **制定标准**: 基于分析，提出具体的评判标准细则，着重于论据是否足够、是否相关、是否有力。
3.  **解释理由**: 为每条标准提供简短的解释 (`explanation`)，说明它为何对于评估此 `<task>` 的论据充分性是重要的。
4.  **分配权重**: 为每条标准分配合理的权重 (`weight`)，确保所有标准的权重之和精确等于 **1.0**。权重应反映该标准对于达成任务论据充分性目标的相对重要性。

**核心要求**：
1.  **紧扣任务**：分析、标准、解释和权重都必须直接关联 `<task>` 的核心要求和特性。
2.  **理由充分**: `<analysis>` 部分需清晰阐述制定这些标准和权重的总体思路，并与 `<task>` 联系。`explanation` 需解释单条标准的针对性。
3.  **标准具体**: 标准应具体、可操作，便于评估时使用。
4.  **标准格式输出**：严格按照下面示例的格式，先输出 `<analysis>` 文本，然后紧接着提供 `<json_output>`。
</instruction>

<example_rational>
以下提供示例，旨在演示**如何根据任务要求制定论据充分性评判标准**。请重点学习这个示例中的**思考逻辑和分析方法**。
</example_rational>
...
<output>
<analysis>
评估针对"预测佛山房价走势及二手房租售决策"任务报告的论据充分性，需要关注支持预测和决策的论据是否全面、有力。该任务要求对房价走势进行预测，并对出售或出租做出决策建议，因此需要多方面的论据支持，包括宏观经济、本地市场、政策环境、历史数据、风险评估等。

具体而言，评估标准需要涵盖：
1.  **论据的全面性**：是否涵盖了影响房价的关键因素（如经济、政策、供需、人口等）以及影响租售决策的因素（如租金回报率、持有成本、交易成本等）。
2.  **论据的相关性**：提供的论据是否直接与佛山房价和租售决策相关，避免无关论据。
3.  **论据的可靠性**：论据是否来自可靠的数据源或权威分析，是否具有时效性。
4.  **论据的数量**：针对每个关键点，是否有足够数量的论据支持，避免单一论据支撑重要论点。
</analysis>
<json_output>
[
  {{
    "criterion": "宏观经济与政策环境论据的充分性",
    "explanation": "评估文章是否提供了足够的宏观经济指标（如GDP、利率）和政策（如房地产调控、城市规划）方面的论据来支持对房价走势的分析。",
    "weight": 0.25
  }},
  {{
    "criterion": "佛山本地市场论据的充分性",
    "explanation": "评估文章是否提供了足够的佛山本地市场数据（如房价历史走势、交易量、供需关系）来支持对佛山房价的预测。",
    "weight": 0.25
  }},
  {{
    "criterion": "租售决策比较论据的充分性",
    "explanation": "评估文章是否提供了足够的关于出售和出租的利弊分析论据（如租金回报率、持有成本、潜在收益、风险）来支持决策建议。",
    "weight": 0.20
  }},
  {{
    "criterion": "论据的时效性",
    "explanation": "评估文章使用的论据（特别是数据）是否具有时效性，能够反映当前市场情况，对于预测和决策至关重要。",
    "weight": 0.15
  }},
  {{
    "criterion": "论据的多样性",
    "explanation": "评估文章是否从多个来源或角度提供论据，避免单一来源的偏见，提高论证的可信度。",
    "weight": 0.15
  }}
]
</json_output>
