...
def should_check_dependency(left: EvidenceRecord, right: EvidenceRecord) -> bool:
    ...
    if _looks_like_explanation(left_text) != _looks_like_explanation(right_text):
        return _shares_financial_topic(left_text, right_text)
    return False
...
def build_dependency_user_prompt(records: list[EvidenceRecord], candidates: list[dict[str, str]]) -> str:
    evidence_items = [
        {
...
async def build_evidence_dependencies(
    ...
    if not candidates:
        return 0
    user_prompt = build_dependency_user_prompt(records, candidates)
