...
class DeepResearchAgent(ReActAgent):
    ...
    async def reply(
        ...
        for _ in range(self.max_iters):
            ...
            reasoning_prompt = prompt_dict["reasoning_prompt"].format_map(
                {
    ...
    async def decompose_and_expand_subtask(self) -> ToolResponse:
        ...
        if len(self.current_subtask) <= self.max_depth:
            ...
            try:
                ...
                gaps_and_plan = {}
                response = prompt_dict["retry_hint"].format_map(
        ...
        return ToolResponse(
            content=[
                TextBlock(
                    type="text",
                    text=prompt_dict["max_depth_hint"],
                ),
            ],
        )
    ...
    async def _follow_up(
        ...
        if len(self.current_subtask) < self.max_depth:
            ...
            expansion_sys_prompt = prompt_dict["deeper_expansion"]
            expansion_inst = prompt_dict["expansion_inst"].format_map(
                {
    ...
    async def summarize_intermediate_results(self) -> ToolResponse:
        ...
        if len(self.intermediate_memory) == 0:
            return ToolResponse(
                content=[
                    TextBlock(
                        type="text",
                        text=prompt_dict["no_result_hint"],
                    ),
                ],
            )
        ...
        if self.intermediate_memory[-1].name == self.summarize_function:
            blocks = await self.get_model_output(
                ...
                + [
                    Msg(
                        "user",
                        prompt_dict["summarize_hint"].format_map(
                            {
        ...
        summarize_sys_prompt = prompt_dict[
            "inprocess_report"
        ].format_map(
        ...
        for item in self.intermediate_memory:
            if isinstance(item.content, str):
                ...
                continue
        summarize_instruction = prompt_dict["summarize_inst"].format_map(
            {
        ...
        if (
            ...
            return ToolResponse(
                content=[
                    TextBlock(
                        type="text",
                        text=prompt_dict["update_report_hint"].format_map(
                            {
                                "intermediate_report": intermediate_report,
            ...
            return ToolResponse(
                content=[
                    TextBlock(
                        type="text",
                        text=prompt_dict["save_report_hint"].format_map(
                            {
                                "intermediate_report": intermediate_report,
                            },
                        ),
    ...
    async def _generate_deepresearch_report(
        ...
        """
        reporting_sys_prompt = prompt_dict["deepresearch_summary_report"]
    ...
    async def reflect_failure(self) -> ToolResponse:
        ...
        """
        reflect_sys_prompt = prompt_dict["reflect_sys_prompt"]
        ...
        reflect_inst = prompt_dict["reflect_instruction"].format_map(
            {
