...
async def judge_consistency_for_evidence_pairs(evidences: List[EvidenceMatch]) -> List[EvidenceComparison]:
    numbered_evidences = "\n".join(
        f"<论据{i + 1}>\n"
        f"主题描述1: {pair[0]}\n"
        f"来源1具体事实: {pair[1]}\n"
...
{{"result_list": {example_json}}}"""
    ...
    if isinstance(comparisons, dict):
        comparisons = EvidenceComparisons(**comparisons)
    return comparisons.result_list
...
async def evidence_coverage_and_accuracy(
    ...
    comparisons = await judge_consistency_for_evidence_pairs(evidence_quads)
